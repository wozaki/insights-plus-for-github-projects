...
2. 設定バーの **Configure** を押して **Start**/**End** の日付フィールドを割り当て、**Save**
3. Start / End 列の日付の横にアラートがインライン表示され、スクロールに追随して更新される

「進行中」「完了」の判定はデフォルトでは Status のオプション名から自動推測されます。上書きするには:

- **Configure** を開き、任意項目の **In Progress statuses** / **Done statuses** を編集（現在の自動判定結果が初期選択済み）
- 一度保存すると以後は選択欄の内容が完全に優先され、選ばれなかった Status はキーワードと一致していてもアラートが出なくなる
- 両方とも空のまま保存（**Clear** リンク使用）すると、自動判定に戻るのではなく Status に基づくアラートが完全にオフになる
