...
### 3. 日付フィールドアラート（リストビュー）

**リストビュー**の Date 型カスタムフィールドに、新しい列を増やさずにセル内へ小さな補助表示を追加し、日付の入力漏れ・作業滞留・期限超過を可視化します。

- ⚠️ **開始/終了の入力漏れ**: 進行中で開始日が未入力の項目、進行中で終了日が未入力の項目、完了で終了日が未入力の項目を検知
- ⏱️ **経過日数（Age）**: 進行中の項目が何日経過しているかを表示（通常 / 注意 / 警告で色分け）
- 🔴 **期限超過（Overdue）**: 未完了で終了日を過ぎた項目を検知
...
1. GitHub Project の**リスト（テーブル）ビュー**（`/views/...`）を開く
2. リスト上部に設定バーが表示される。**Configure** を押して **Start**（開始日）と **End**（終了日）のフィールドを割り当て（Date 型フィールドから候補を自動選択）、**Save** を押す
3. Start / End 列の日付の横にアラートがインライン表示され、スクロールに追随して更新される

項目が「進行中」か「完了」かは、**Configure** を一度も開かない限りは Status のオプション名（例: "In Progress"、"Review"、"Done"）から自動判定されます。Status の名称が独自のものだったり、選択肢が多く（10個程度など）自動判定が信頼できない場合は、**Configure** を開いて任意項目の **In Progress statuses** / **Done statuses** から明示的に選択してください。両方とも現在の自動判定結果が初期選択されているので、間違っている部分だけ直せば済みます。一度保存すると、以後は選択欄の内容が完全に優先されます。両方に含まれない Status はキーワードと一致していても「未分類」としてアラートが一切出なくなります。**両方とも空のまま保存した場合も同様**（各選択欄上の **Clear** リンクで一括解除できます。`<select multiple>` は標準機能だけでは全解除しづらいため）— これは自動判定に戻るのではなく、Status に基づくアラートを完全にオフにする、という意味になります。

#### アラート条件

| Status | 条件 | 表示 |
|--------|------|------|
| 進行中 | 開始日が未入力 | Start に `⚠ Missing` |
| 進行中 | 開始日が過去 | Start に `Age Nd`（色分け: 0〜5 通常 / 6〜10 注意 / 11以上 警告） |
| Todo または進行中 | 終了日が過去 | End に `Overdue Nd` |
| 進行中 | 終了日が未入力 | End に `⚠ Missing` |
| 完了 | 終了日が未入力 | End に `⚠ Missing` |

補足:

- Start / End のフィールド割り当てとStatusの任意設定は、プロジェクトごとに `chrome.storage.local` に保存されます。内部では Field ID / Status ID を使うため、名称を変更しても動作します。
- アラートはページ読み込み時の項目から算出され、リロードで最新化されます（読み込み後の編集はリロード後に反映）。
