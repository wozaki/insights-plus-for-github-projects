...
項目が「進行中」か「完了」かは、デフォルトでは Status のオプション名（例: "In Progress"、"Review"、"Done"）から自動判定されます。Status の名称が独自のものだったり、選択肢が多く（10個程度など）自動判定が信頼できない場合は、**Configure** を開いて任意項目の **In Progress statuses** / **Done statuses** から明示的に選択してください。両方とも現在の自動判定結果が初期選択されているので、間違っている部分だけ直せば済みます。両方とも空のままにすれば自動判定のままになります。
...
| 未完了（完了以外すべて） | 終了日が過去 | End に `Overdue Nd` |
